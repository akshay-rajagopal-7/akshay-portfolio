...
A personal portfolio website for Akshay Rajagopal (Finance · Accounts · Compliance, Dubai UAE). Dark "forensic brutalism" design. It showcases four works and links out to LinkedIn + email.
...
2. **Darvin & the Pulse** (Beta) — 3-agent AI review tribunal (Darvin / Ward / Athena). Demo "coming soon".
...
## 3. How to update the portfolio site

The whole site is one file: `index.html`. To change anything (text, design, links):

1. Get the new `index.html` (e.g. download it).
2. In Terminal:
   ```bash
   cd ~/Documents/akshay-portfolio
   cp "$(ls -t ~/Downloads/*.html | head -1)" index.html   # copies newest .html from Downloads
   git add index.html
...
## 4. Railway — the AureviaForge live app
...
- **Real-world cost:** the app uses ~$1/month of compute, which sits inside the free $5 credit. So the expected bill is just the **$5 base**, ~$60/year.
...
- **Compute hard limit: $10** — when compute spend hits $10 in a billing cycle, **all resources stop** (app goes offline until next cycle or you raise it). $10 is the lowest Railway allows.
...
Railway auto-named the project **`aware-wisdom`** (random). That's the project that holds AureviaForge. Don't be confused by the name.

### How to bring the app back online (after a sleep / cap shutoff / trial expiry)
1. railway.com → log in.
2. Left sidebar → **Projects** → open **aware-wisdom**.
3. Click the service tile. If it's stopped/crashed/sleeping → **Deploy** (or ⋯ → Redeploy / Restart).
...
Settings → Billing → cancel/downgrade. Then either:
- change the portfolio's "Open live app" button to "demo on request", or
- move the app to a free host (see §5).

---

## 5. IMPORTANT — public upload risk (unresolved)

**The problem:** AureviaForge currently lets any visitor upload data. Processing large files (e.g. 50 million rows) makes the server work hard (CPU/RAM spike), which:
...
1. **Cap upload size** in the FastAPI backend (e.g. reject files over ~10k–50k rows for the public demo). Best balance. Requires a code change in the `aureviaforge-engine` repo.
...
## 6. AureviaForge — technical summary
...
## 7. Darvin & the Pulse — technical summary
...
- **Status:** Beta. Pushed to GitHub (public). Demo not yet deployed. Could be hosted free on Render later to make the demo link live.
- **API keys:** never commit them. `.gitignore` already excludes `.env`. Keys are read via `os.getenv()` only.

---

## 8. Maintenance reminders

- **"Available for work" badge** in the footer — turn it off / change wording once a role is landed, so it stays honest. Edit `index.html`, search `Available`.
- **Keep claims accurate** — the "100 million transactions," 10 tests, etc. should all be defensible if a technical interviewer probes.
...
## 9. Optional upgrades (not done yet)
...
- **Upload size cap / sample-data mode** for AureviaForge (see §5) — do before sharing widely.
